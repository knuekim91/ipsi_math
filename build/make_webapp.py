...
.sheet .opt.primary{background:var(--indigo);border-color:var(--indigo);color:#fff;font-weight:500}
.sheet .opt.primary small{color:rgba(255,255,255,.75)}
...
<div class="modal" id="askmodal">
  <div class="sheet">
    <h2>엄마에게 보내기</h2>
    <div class="sub" id="asksub">아래 내용을 엄마에게 보내면, 엄마가 Claude에게 전달해 문항을 만들어 옵니다.</div>
    <button class="opt primary" id="asksend">카카오톡 · 메시지로 보내기<small>공유 창이 열리면 엄마를 고르세요</small></button>
    <button class="opt" id="askcopy">복사하기<small>복사한 뒤 직접 붙여넣어도 됩니다</small></button>
    <div class="askbox" id="askbox"></div>
    <div class="hint">위 상자를 길게 눌러 직접 복사하거나, 화면을 캡처해 보내도 됩니다.</div>
...
(function(){
  ...
  if (!navigator.share) {                 // 공유를 못 쓰는 기기(대개 PC)면 감춘다
    btn.style.display = 'none';
    document.getElementById('asksub').textContent =
      '아래 내용을 복사해 Claude 대화창에 붙여넣으면, 이 메모에 맞춘 문항을 만들어 드립니다.';
    return;
  }
  btn.onclick = function(){
    const txt = document.getElementById('askbox').textContent;
    navigator.share({ title: 'ipsi_math 질문', text: txt }).then(function(){
      document.getElementById('askmodal').classList.remove('on');
      toast('보냈어요');
    }, function(err){
      if (err && err.name === 'AbortError') return;      // 사용자가 취소한 것
      copyText(txt, '공유가 막혀 있어 복사했어요. 붙여넣어 보내세요');
    });
  };
})();
