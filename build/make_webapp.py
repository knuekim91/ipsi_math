...
@media print{
  ...
  html,body{background:#fff!important;color:#000!important;padding:0!important;font-size:10.6pt}
  .wrap,.bar,.modal,.toast{display:none!important}
  #printarea{display:block}
  .pr-head{display:flex;justify-content:space-between;align-items:flex-end;
    border-bottom:1.6pt solid #000;padding-bottom:5pt;margin-bottom:12pt}
  .pr-t{font-size:15pt;font-weight:700}
  .pr-m{font-size:8pt;color:#444}
  .pr-q{page-break-inside:avoid;margin-bottom:11pt}
  .pr-sol{page-break-inside:auto;border-bottom:.5pt solid #ccc;padding-bottom:7pt}
  .pr-sol .pr-qh{page-break-after:avoid}
  .pr-qh{display:flex;gap:6pt;align-items:baseline;border-bottom:.6pt solid #999;
    padding-bottom:3pt;margin-bottom:6pt}
  .pr-n{font-size:14pt;font-weight:700;font-family:Georgia,serif;min-width:16pt}
  .pr-c{font-size:7.6pt;border:.6pt solid #999;padding:.5pt 4pt;white-space:nowrap}
  .pr-s p{margin:0 0 5pt}
  .pr-s .cond{display:block;margin:5pt 0;padding:4pt 9pt;border:.6pt solid #666}
  .pr-s .choices{display:flex;flex-wrap:wrap;gap:4pt 20pt;margin-top:6pt}
  .pr-w{border:.5pt dashed #bbb;margin-top:7pt}
  .pr-break{page-break-after:always}
  .pr-a{font-size:9.4pt;font-weight:700;border:1pt solid #000;padding:1pt 7pt;margin-left:auto}
  .pr-b{margin-bottom:6pt;font-size:10pt}
  .pr-b .lab{display:inline-block;font-size:7.8pt;font-weight:700;letter-spacing:.18em;
    margin-right:7pt;border-bottom:1.4pt solid #000}
  .pr-b.idea{padding-left:8pt;border-left:2.2pt solid #000}
  .pr-b.alt{padding:6pt 9pt;border:.6pt dashed #666}
  .pr-b.trap{padding:5pt 9pt;border-left:2.2pt solid #777}
  .pr-b.know{padding:6pt 9pt;border:1pt solid #000}
  .pr-b p{margin:0 0 4pt}
}
</style>
...
<div class="modal" id="pmodal">
  <div class="sheet">
    ...
    <button class="opt" id="opt1">문제만 인쇄<small>풀이 공간이 함께 나옵니다. 시험지처럼 풀 때</small></button>
    <button class="opt" id="opt2">문제 + 해설 인쇄<small>문제를 먼저 싣고, 쪽을 나눠 해설을 붙입니다</small></button>
...
function doPrint(withSol){
  document.getElementById('printarea').innerHTML = buildPrint(withSol);
  document.getElementById('pmodal').classList.remove('on');
  setTimeout(function(){
    try { window.print(); }
    catch (e) { toast('인쇄를 열 수 없어요. 문항 번호 복사를 써주세요'); }
  }, 60);
}
document.getElementById('opt1').onclick = function(){ doPrint(false); };
document.getElementById('opt2').onclick = function(){ doPrint(true); };
document.getElementById('opt3').onclick = function(){
  const NL = String.fromCharCode(10);
  const txt = selectedProblems().map(function(p){ return p.id; }).join(NL);
  const done = function(){ toast('문항 번호 ' + selected.size + '개를 복사했어요');
                           document.getElementById('pmodal').classList.remove('on'); };
...
function fallbackCopy(txt, done){
  ...
  catch (e) { toast('복사가 안 돼요. 화면의 문항 번호를 직접 적어주세요'); }
