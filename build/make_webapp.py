...
.fchip{flex:none;font-size:12.5px;padding:5px 12px;border:1px solid var(--rule);
...
.rbtn{font-size:12.5px;padding:6px 13px;border:1px solid var(--rule);background:var(--card);
...
.mbtn{flex:1;font-size:13px;padding:9px 4px;border:1px solid var(--rule);background:var(--card);
  color:var(--ink2);cursor:pointer;font-family:inherit;border-radius:2px}
...
.note{margin-top:9px;width:100%;font-family:inherit;font-size:13.5px;padding:9px 11px;
  border:1px solid var(--rule);background:var(--paper);color:var(--ink);resize:vertical;min-height:40px}
...
.barbtn{font-size:12.5px;padding:7px 14px;border:1px solid var(--ink);background:var(--ink);
  color:var(--paper);cursor:pointer;font-family:inherit;border-radius:2px}
...
.sheet{background:var(--card);width:100%;max-width:720px;max-height:82vh;overflow-y:auto;
  border-top:2px solid var(--ink);padding:20px 18px 28px}
...
.close{margin-top:18px;width:100%;font-size:13.5px;padding:11px;border:1px solid var(--rule);
...
<div class="bar">
  <div class="barin">
    ...
    <button class="barbtn" id="reviewbtn">복습할 문항 보기</button>
  </div>
</div>
...
function renderSummary(){
  ...
  document.getElementById('barinfo').innerHTML =
    '아직 안 푼 문항 ' + left + '개 · 다시 볼 문항 <b>' + (mid + no) + '</b>개';
